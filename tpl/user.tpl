...
count = 100;
  ...
  $.getJSON("./action.php"
  ,{ "act":"userListGetJson", "login_id":login_id, "offset":offset, "count":count }
...
// display userList 
function appendUerList(data) {

  // append user list to table
  $("#userTable").append(
    '<tr id="' + user_json[data]['user_id'] + '">'
     +"<td>"
       + '<input type="checkbox" id="userdeletecheck" value="'
         + user_json[data]['login_id'] + '"/>'
         +"&nbsp"
             + user_json[data]['login']     + "</td>"
     +"<td>" + user_json[data]['authority'] + "</td>"
   +"</tr>"
  );
}
